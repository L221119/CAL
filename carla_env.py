...
Supports training on four separate tasks.
"""
...
def parse_args():
    ...
    parser.add_argument('--task', type=str, default='straight',
                       choices=['straight', 'left', 'right', 'uturn', 'all'],
                       help='Task to train on: straight, left, right, uturn, or all')
...
def save_checkpoint(agent, episode, save_path, task, is_final=False):
    ...
    if is_final:
        filename = os.path.join(save_path, f'final_model_{task}.pth')
    else:
        filename = os.path.join(save_path, f'checkpoint_{task}_{episode}.pth')
...
def train_single_task(args, config, task):
    ...
    # Setup directories
    save_path, log_dir = setup_directories(args.save_path, args.tensorboard_log, task)
    ...
    csv_path = os.path.join('date', f'train_{task}_{run_id}.csv')
    csv_file = open(csv_path, 'w', newline='')
    csv_writer = csv.writer(csv_file)
    csv_writer.writerow(['episode', 'task', 'total_reward', 'avg_reward', 
                        'success', 'collision', 'avg_speed', 'completion_time', 'weather'])
    
    # Initialize environment with specific task
    env = CarlaIntersectionEnv(config, args.host, args.port, task=task)
    env.setup()
    ...
    for episode in range(1, args.total_episodes + 1):
        ...
        csv_writer.writerow([
            ...
            info.get('weather', 'unknown')
        ])
        csv_file.flush()
        
        # TensorBoard logging
        writer.add_scalar(f'{task}/Episode/Total_Reward', episode_reward, episode)
        writer.add_scalar(f'{task}/Episode/Steps', episode_steps, episode)
        writer.add_scalar(f'{task}/Episode/Success', float(info.get('success', False)), episode)
        writer.add_scalar(f'{task}/Episode/Collision', float(info.get('collision', False)), episode)
        writer.add_scalar(f'{task}/Episode/Avg_Speed', info.get('avg_speed', 0.0), episode)
        
        # Print progress
        if episode % 10 == 0:
            print(f'[{task}] Episode {episode}/{args.total_episodes}, '
        ...
        if episode % args.eval_freq == 0:
            ...
            writer.add_scalar(f'{task}/Evaluation/Avg_Reward', eval_results['avg_reward'], episode)
            writer.add_scalar(f'{task}/Evaluation/Success_Rate', eval_results['success_rate'], episode)
            writer.add_scalar(f'{task}/Evaluation/Collision_Rate', eval_results['collision_rate'], episode)
            
            print(f'\n=== [{task}] Evaluation at Episode {episode} ===')
            ...
            if eval_results['success_rate'] > best_success_rate:
                ...
                save_checkpoint(agent, episode, save_path, task, is_final=False)
    ...
    # Save final model
    save_checkpoint(agent, args.total_episodes, save_path, task, is_final=True)
...
def train_all_tasks(args, config):
    """Train on all tasks sequentially."""
    tasks = ['straight', 'left', 'right', 'uturn']
    results = {}
    
    for task in tasks:
        results[task] = train_single_task(args, config, task)
    
    print('\n' + '='*60)
    print('Training Summary')
    print('='*60)
    for task, success_rate in results.items():
        print(f'{task}: Best Success Rate = {success_rate:.2f}%')
...
def main():
    ...
    if args.task == 'all':
        train_all_tasks(args, config)
    else:
        train_single_task(args, config, args.task)
